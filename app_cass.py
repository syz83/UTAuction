...
class User(UserMixin):
	...
	def get(userid):
		...
		user = session.execute(select_statement, [userid])[0]
		if user:
			return User(user.username, user.password, user.watch_list)
		return None
...
def detail(_id):
	...
	item = session.execute("SELECT * FROM items WHERE id=" + _id)[0]
